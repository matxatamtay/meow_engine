...
#[test]
fn vertical_flow_fixtures_are_byte_stable() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/vertical-flow");
    let mut fixtures = fs::read_dir(&root)
        .expect("vertical-flow fixture directory should exist")
    ...
    for fixture in fixtures {
        ...
        let actual = layout_normal_flow(&boxes, &styles, LayoutViewport::new(480, 640)).dump();
        let expected_path = fixture.join("expected.dump");
        if std::env::var_os("UPDATE_W15_SNAPSHOTS").is_some() {
